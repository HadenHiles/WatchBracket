...
import { displaySessions, mediaItems, participants, rooms, submissions } from '@watch-bracket/db';
...
export async function getSnapshot(db: Database, roomId: string, viewer: RoomSnapshot['viewer'], presence: Presence, viewerParticipantId?: string): Promise<RoomSnapshot> {
  ...
  const selected = await db.select({ participantId: submissions.participantId, rank: submissions.rank, catalogKey: mediaItems.catalogKey, mediaType: mediaItems.mediaType, title: mediaItems.title, releaseYear: mediaItems.releaseYear, runtimeMinutes: mediaItems.runtimeMinutes, contentRating: mediaItems.contentRating, genres: mediaItems.genres, synopsis: mediaItems.synopsis, posterUrl: mediaItems.posterUrl, metadata: mediaItems.metadata })
    .from(submissions).innerJoin(mediaItems, eq(submissions.mediaItemId, mediaItems.id)).where(eq(submissions.roomId, roomId));
  const asCatalogItem = (item: typeof selected[number]): CatalogItem => { const metadata=item.metadata&&typeof item.metadata==='object'?item.metadata as Record<string,unknown>:{};const availability=MediaAvailabilitySchema.safeParse(metadata.availability);const local=parseLocalAvailability(metadata.localAvailability);const request=parseRequestAvailability(metadata.requestAvailability);return { catalogKey: item.catalogKey, mediaType: item.mediaType, title: item.title, releaseYear: item.releaseYear, runtimeMinutes: item.runtimeMinutes ?? 0, contentRating: item.contentRating ?? 'Unrated', genres: Array.isArray(item.genres) ? item.genres.filter((genre): genre is string => typeof genre === 'string') : [], synopsis: item.synopsis,posterUrl:item.posterUrl,...(availability.success?{availability:availability.data}:{}),...(local?{localAvailability:local}:{}),...(request?{requestAvailability:request}:{}) };};
  ...
  if (revealed) for (const item of selected) {
    ...
    if (current) { current.supportCount += 1; current.bestRank = Math.min(current.bestRank, item.rank); }
    else candidateMap.set(item.catalogKey, { ...asCatalogItem(item), supportCount: 1, bestRank: item.rank });
  }
  return {
    roomId: room.id, name: room.name, code: room.code, state: room.state, locked: Boolean(room.lockedAt), sequence: room.version, viewer, viewerParticipantId: viewer === 'DISPLAY' ? null : viewerParticipantId ?? null, viewerReady: people.find((person) => person.id === viewerParticipantId)?.ready ?? false,
...
function parseLocalAvailability(value:unknown){if(!value||typeof value!=='object')return undefined;const item=value as Record<string,unknown>;if(typeof item.available!=='boolean')return undefined;return{available:item.available,plexUrl:typeof item.plexUrl==='string'?item.plexUrl:null,libraryTitle:typeof item.libraryTitle==='string'?item.libraryTitle:null,episodeCount:typeof item.episodeCount==='number'?item.episodeCount:null};}
function parseRequestAvailability(value:unknown){if(!value||typeof value!=='object')return undefined;const item=value as Record<string,unknown>;const statuses=['UNKNOWN','PENDING','PROCESSING','PARTIAL','AVAILABLE','REQUESTABLE','UNAVAILABLE'] as const;const status=statuses.find((entry)=>entry===item.status);if(!status||typeof item.requestable!=='boolean')return undefined;return{status,requestable:item.requestable};}
