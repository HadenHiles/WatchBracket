...
export default function Home() {
  ...
  const [authenticated, setAuthenticated] = useState(false); const [loading, setLoading] = useState(true); const [error, setError] = useState('');
  ...
  useEffect(() => { setCreateIntent(new URLSearchParams(window.location.search).get('create') === '1'); api<{authenticated:boolean}>('/api/auth/session').then((s) => setAuthenticated(s.authenticated)).finally(() => setLoading(false)); }, []);
  async function create() {
    if (!authenticated) { router.push('/admin/login?next=%2F%3Fcreate%3D1'); return; }
