...
export default function Home() {
  ...
  const [plexConnected, setPlexConnected] = useState(false); const [plexAccountLabel, setPlexAccountLabel] = useState<string>();
  ...
  useEffect(() => {
    setHydrated(true);
    api<{authenticated:boolean}>('/api/auth/session').then((session)=>setAuthenticated(session.authenticated)).catch(()=>setAuthenticated(false));
    const cached = window.sessionStorage.getItem('watch-bracket:plex:session');
    if (cached) { setPlexConnected(true); setPlexAccountLabel(cached); }
    api<{connected:boolean;accountLabel:string|null}>('/api/plex/status').then((status) => {
      setPlexConnected(status.connected);
      setPlexAccountLabel(status.accountLabel ?? undefined);
      if (status.connected) window.sessionStorage.setItem('watch-bracket:plex:session', status.accountLabel ?? 'Plex connected');
      else window.sessionStorage.removeItem('watch-bracket:plex:session');
    }).catch(() => undefined);
  }, []);
  ...
  return <main className="shell home-shell"><header className="home-header"><div className="site-logo"><Image src="/brand/watch-bracket-wordmark.png" alt="Watch Bracket" fill priority sizes="(max-width: 680px) 72vw, 360px"/></div><p>Feature presentation selector</p></header><section className="hero home-hero"><div className="hero-copy"><h1>Tonight&apos;s pick, decided together.</h1><p className="lead">Create a room, invite the couch, and turn the endless scroll into a quick shared decision.</p></div></section>
    ...
    <div className="two-col">
      <section className="card stack"><div className="card-heading"><Image className="card-icon" src="/brand/vhs-tape-icon.png" alt="" width={112} height={75}/><div><p className="kicker">Tonight&apos;s feature</p><h2>Create a room</h2></div></div><label>Room name<input value={roomName} maxLength={80} onChange={(e)=>setRoomName(e.target.value)} /></label><label>Your nickname<input value={hostNickname} maxLength={32} onChange={(e)=>setHostNickname(e.target.value)} /></label><button onClick={()=>void create(false)} disabled={!hydrated || creating}>{creating?'Creating…':'Create a room'}</button><button className={`plex-button${plexConnected ? ' connected' : ''}`} onClick={()=>void create(!plexConnected)} disabled={!hydrated || creating}><Image src="/brand/plex-logo.svg" alt="Plex" width={92} height={28}/><span>{plexConnected ? 'Create with my Plex profile' : 'Create + personalize with Plex'}</span></button><small className="muted" role={plexConnected ? 'status' : undefined}>{plexConnected ? `${plexAccountLabel ?? 'Plex account'} connected · your profile will follow you into the room.` : 'Plex is optional. It adds your watchlist and taste-based suggestions.'}</small></section>
