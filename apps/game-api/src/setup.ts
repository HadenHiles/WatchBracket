...
export async function getHouseholdSetup(ctx: DomainContext) {
  ...
  if (!household)
    throw new DomainError(
      "HOUSEHOLD_MISSING",
      "Household configuration is missing.",
      500,
    );
  return {
    id: household.id,
    ...
    defaultRules: HouseRulesSchema.parse(household.defaultRules),
    historyEnabled: household.historyEnabled,
    recentExclusionDays: household.recentExclusionDays,
    completed: Boolean(household.onboardingCompletedAt),
  };
}

export async function saveHouseholdSetup(
  ctx: DomainContext,
  adminId: string,
  input: {
    name: string;
  ...
  try {
    ...
    throw new DomainError(
      "TIME_ZONE_INVALID",
      "Use a valid IANA time zone, such as America/Toronto.",
      400,
    );
  }
  const [updated] = await ctx.db
    ...
    .set({
      ...
      recentExclusionDays: input.recentExclusionDays,
  ...
  await ctx.db
    ...
    .values({
      ...
      actorType: "ADMIN",
      actorId: adminId,
      eventType: "HOUSEHOLD_SETUP_UPDATED",
      metadata: {
        region: input.region.toUpperCase(),
        timeZone: input.timeZone,
        preset: rules.preset,
