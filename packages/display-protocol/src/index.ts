...
const TournamentCandidateSceneSchema=z.object({id:z.uuid(),title:z.string(),mediaType:z.enum(['MOVIE','TV']),releaseYear:z.number().int(),runtimeMinutes:z.number().int().nonnegative(),contentRating:z.string(),genres:z.array(z.string()),posterUrl:z.url().nullable().optional(),availability:z.object({region:z.string().length(2),link:z.url().nullable(),attribution:z.literal('JustWatch'),offers:z.array(z.object({providerId:z.number().int().positive(),providerName:z.string(),logoUrl:z.url().nullable(),category:z.enum(['SUBSCRIPTION','FREE','ADS','RENT','BUY'])}))}).optional(),seed:z.number().int().positive(),strikes:z.number().int().nonnegative(),redemption:z.boolean()});
