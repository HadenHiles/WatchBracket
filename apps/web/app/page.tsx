...
export default function Home() {
  ...
  return <main className="shell home-shell"><header className="home-header"><div className="site-logo"><Image src="/brand/watch-bracket-wordmark.png" alt="Watch Bracket" fill priority sizes="(max-width: 680px) 72vw, 360px"/></div><p>Feature presentation selector</p></header><section className="hero home-hero"><div className="hero-copy"><h1>Tonight&apos;s pick, decided together.</h1><p className="lead">Create a room, invite the couch, and turn the endless scroll into a quick shared decision.</p></div></section>
    ...
    <div className="two-col">
      <section className="card stack"><div className="card-heading"><Image className="card-icon" src="/brand/vhs-tape-icon.png" alt="" width={112} height={75}/><div><p className="kicker">Tonight&apos;s feature</p><h2>Create a room</h2></div></div><label>Room name<input value={roomName} maxLength={80} onChange={(e)=>setRoomName(e.target.value)} /></label><label>Your nickname<input value={hostNickname} maxLength={32} onChange={(e)=>setHostNickname(e.target.value)} /></label><button onClick={create} disabled={!hydrated || creating}>{creating?'Creating…':'Create a Room'}</button><small className="muted">No account required. This browser becomes the room host.</small>{authenticated?<><a className="button secondary" href="/setup">Setup &amp; integrations</a><a href="/admin/recommendations">Recommendation diagnostics</a><button className="secondary" onClick={signOut}>Sign out</button></>:<a href="/admin/login?next=%2Fsetup">Server settings</a>}</section>
