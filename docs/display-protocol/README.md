...
Milestone 4 adds validated matchup intro, voting, result, and winner scenes. Voting scenes contain only the aggregate received/eligible count; the aggregate vote split appears only after server resolution. Redemption status and the winner journey are semantic data, so browser and Cast displays remain equivalent.
