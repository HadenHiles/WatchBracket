...
When household history is enabled, wildcard generation ranks recent winners and candidates behind unseen recommendations for the configured 0–365 day window. If too few unseen titles remain to form the requested bracket, recent candidates are admitted only as last-resort fillers so the game remains playable. The setup screen can disable this behavior or permanently clear all Watch Bracket history and derived taste summaries.
