...
describe('TmdbProvider', () => {
  it('normalizes canonical identity, artwork, ratings, and distinct Canadian availability categories', async () => {
    const fakeFetch = (async (input: URL | RequestInfo) => {
      ...
      if (url.pathname === '/3/movie/11') return json({
        id: 11, title: 'Example', original_title: 'Example Original', original_language: 'en', release_date: '2024-03-01', overview: 'A complete example.', runtime: 112,
    ...
    expect(item).toMatchObject({ catalogKey: 'tmdb:MOVIE:11', tmdbId: 11, title: 'Example', originalLanguage: 'en', releaseYear: 2024, runtimeMinutes: 112, contentRating: 'PG-13' });
  ...
  it('coalesces identical requests and caches their result', async () => {
    ...
    const fakeFetch = (async () => {
      ...
      return json({
        id: 11, title: 'Example', original_title: 'Example', original_language: 'en', release_date: '2024-03-01', overview: '', runtime: 100,
