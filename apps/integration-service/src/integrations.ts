    ...
    for (const library of libraries) {
      ...
      for (const entry of array(raw?.Metadata)) {
        ...
        items.push(PlexInventoryItemSchema.parse({
          ...
          plexUrl: machineIdentifier ? `https://app.plex.tv/desktop/#!/server/${encodeURIComponent(machineIdentifier)}/details?key=${encodeURIComponent(`/library/metadata/${ratingKey}`)}` : null,
...
export class SeerrProvider {
  ...
  async request(input: { tmdbId: number; mediaType: 'MOVIE' | 'TV'; tvSeasonPolicy?: 'FIRST' | 'LATEST' | 'ALL' | undefined }) {
    ...
    if (input.mediaType === 'TV') payload.seasons = input.tvSeasonPolicy === 'ALL' ? 'all' : input.tvSeasonPolicy === 'LATEST' ? 'latest' : [1];
