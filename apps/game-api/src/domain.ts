...
export class DomainError extends Error { constructor(public code: string, message: string, public status = 400, public details?: Record<string, unknown>) { super(message); } }
...
export async function bootstrapAdmin({ db, env }: DomainContext) {
  ...
  const passwordHash = await hashPassword(env.ADMIN_BOOTSTRAP_PASSWORD, { algorithm: Algorithm.Argon2id, memoryCost: 19_456, timeCost: 2, parallelism: 1 });
  return db.transaction(async (tx) => {
    const recheck = await tx.select({ id: adminUsers.id }).from(adminUsers).limit(1);
    if (recheck.length) return { created: false, adminId: recheck[0]!.id };
    const [admin] = await tx.insert(adminUsers).values({ email: env.ADMIN_BOOTSTRAP_EMAIL, passwordHash }).returning({ id: adminUsers.id });
    await tx.insert(households).values({ name: 'Watch Bracket Household' });
    return { created: true, adminId: admin!.id };
  });
}

export async function login({ db, env }: DomainContext, email: string, password: string) {
  const normalized = email.trim().toLowerCase();
  const [admin] = await db.select().from(adminUsers).where(eq(adminUsers.email, normalized)).limit(1);
  if (!admin || !(await verifyPassword(admin.passwordHash, password))) throw new DomainError('INVALID_CREDENTIALS', 'Invalid email or password.', 401);
  const token = generateSessionToken();
  const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
  await db.transaction(async (tx) => {
    await tx.update(adminSessions).set({ revokedAt: new Date() }).where(and(eq(adminSessions.adminUserId, admin.id), isNull(adminSessions.revokedAt)));
    await tx.insert(adminSessions).values({ adminUserId: admin.id, tokenHash: hashToken(token, env.HOST_SESSION_PEPPER), expiresAt });
    await tx.update(adminUsers).set({ lastLoginAt: new Date(), updatedAt: new Date() }).where(eq(adminUsers.id, admin.id));
  });
...
export async function resolveAdmin({ db, env }: DomainContext, token?: string) {
  ...
  const [row] = await db.select({ sessionId: adminSessions.id, adminId: adminUsers.id, email: adminUsers.email })
    .from(adminSessions).innerJoin(adminUsers, eq(adminSessions.adminUserId, adminUsers.id))
    .where(and(eq(adminSessions.tokenHash, hashToken(token, env.HOST_SESSION_PEPPER)), isNull(adminSessions.revokedAt), gt(adminSessions.expiresAt, new Date()))).limit(1);
  if (row) await db.update(adminSessions).set({ lastSeenAt: new Date() }).where(eq(adminSessions.id, row.sessionId));
  return row;
}

export async function resolveParticipant({ db, env }: DomainContext, token?: string) {
  if (!token) return undefined;
  const [row] = await db.select().from(participants).where(and(eq(participants.tokenHash, hashToken(token, env.PARTICIPANT_SESSION_PEPPER)), isNull(participants.removedAt))).limit(1);
  return row;
}
export async function resolveDisplay({ db, env }: DomainContext, token?: string) {
  if (!token) return undefined;
  const [row] = await db.select().from(displaySessions).where(and(eq(displaySessions.tokenHash, hashToken(token, env.DISPLAY_SESSION_PEPPER)), isNull(displaySessions.revokedAt), gt(displaySessions.expiresAt, new Date()))).limit(1);
  return row;
}

export async function logout({ db, env }: DomainContext, token?: string) {
  if (token) await db.update(adminSessions).set({ revokedAt: new Date() }).where(eq(adminSessions.tokenHash, hashToken(token, env.HOST_SESSION_PEPPER)));
}
...
export async function createRoom(ctx: DomainContext, adminId: string, body: { name: string; hostNickname: string }, idempotencyKey: string) {
  ...
  if (!household) throw new DomainError('HOUSEHOLD_MISSING', 'Default household is not configured.', 500);
  ...
  throw new DomainError('ROOM_CODE_EXHAUSTED', 'Could not allocate a room code. Try again.', 503);
}
...
export async function joinRoom(ctx: DomainContext, body: { roomCode: string; nickname: string }, existingToken?: string) {
  ...
  try {
    const [participant] = await db.transaction(async (tx) => {
      const inserted = await tx.insert(participants).values({ roomId: room.id, normalizedNickname: nickname.normalized, displayNickname: nickname.display, tokenHash: hashToken(token, env.PARTICIPANT_SESSION_PEPPER) }).returning();
...
export async function requireRoomHost(db: Database, participantId: string, roomId: string) {
  const [room] = await db.select().from(rooms).where(eq(rooms.id, roomId)).limit(1);
  if (!room || room.hostParticipantId !== participantId) throw new DomainError('HOST_REQUIRED', 'Room host authorization is required.', 403);
  return room;
}

export async function setRoomLock(db: Database, participantId: string, roomId: string, locked: boolean) {
  const room = await requireRoomHost(db, participantId, roomId);
  if (room.state !== 'LOBBY') throw new DomainError('ROOM_NOT_ACTIVE', 'The room is not active.', 409);
  const [updated] = await db.update(rooms).set({ lockedAt: locked ? new Date() : null, version: sql`${rooms.version} + 1`, updatedAt: new Date() }).where(eq(rooms.id, roomId)).returning();
  await db.insert(auditEvents).values({ householdId: room.householdId, roomId, actorType: 'PARTICIPANT', actorId: participantId, eventType: locked ? 'ROOM_LOCKED' : 'ROOM_UNLOCKED' });
  return updated!;
}

export async function createDisplayPairingCode(ctx: DomainContext, participantId: string, roomId: string) {
  const room = await requireRoomHost(ctx.db, participantId, roomId);
  const code = generatePairingCode();
  const expiresAt = new Date(Date.now() + ctx.env.DISPLAY_PAIRING_TTL_SECONDS * 1000);
...
export async function pairDisplay(ctx: DomainContext, body: { pairingCode: string; displayName: string }) {
  const { db, env } = ctx;
  const codeHash = hashToken(body.pairingCode.trim(), env.DISPLAY_SESSION_PEPPER);
  const outcome = await db.transaction(async (tx) => {
    const [pairing] = await tx.select().from(displayPairingCodes).where(eq(displayPairingCodes.codeHash, codeHash)).for('update').limit(1);
    if (!pairing) return { error: new DomainError('PAIRING_CODE_INVALID', 'The pairing code is invalid.', 400) };
    await tx.update(displayPairingCodes).set({ attemptCount: sql`${displayPairingCodes.attemptCount} + 1` }).where(eq(displayPairingCodes.id, pairing.id));
    ...
    const [display] = await tx.insert(displaySessions).values({ roomId: room.id, displayName: body.displayName.trim(), tokenHash: hashToken(token, env.DISPLAY_SESSION_PEPPER), pairedByParticipantId: room.hostParticipantId, expiresAt }).returning();
    await tx.update(displayPairingCodes).set({ consumedAt: new Date() }).where(eq(displayPairingCodes.id, pairing.id));
    await tx.update(rooms).set({ version: sql`${rooms.version} + 1` }).where(eq(rooms.id, room.id));
    await tx.insert(auditEvents).values({ householdId: room.householdId, roomId: room.id, actorType: 'PARTICIPANT', actorId: room.hostParticipantId, eventType: 'DISPLAY_PAIRED', metadata: { displaySessionId: display!.id, kind: 'BROWSER' } });
...
export async function revokeDisplay(db: Database, participantId: string, displaySessionId: string) {
  const [display] = await db.select().from(displaySessions).where(eq(displaySessions.id, displaySessionId)).limit(1);
  if (!display) throw new DomainError('DISPLAY_NOT_FOUND', 'Display session not found.', 404);
  const room = await requireRoomHost(db, participantId, display.roomId);
  await db.transaction(async (tx) => {
    await tx.update(displaySessions).set({ revokedAt: new Date() }).where(and(eq(displaySessions.id, display.id), isNull(displaySessions.revokedAt)));
    await tx.update(rooms).set({ version: sql`${rooms.version} + 1` }).where(eq(rooms.id, room.id));
    await tx.insert(auditEvents).values({ householdId: room.householdId, roomId: room.id, actorType: 'PARTICIPANT', actorId: participantId, eventType: 'DISPLAY_REVOKED', metadata: { displaySessionId } });
  });
  return display;
}

export async function bumpRoomVersion(db: Database, roomId: string) {
  const [room] = await db.update(rooms).set({ version: sql`${rooms.version} + 1`, updatedAt: new Date() }).where(eq(rooms.id, roomId)).returning({ version: rooms.version });
  return room?.version;
}
